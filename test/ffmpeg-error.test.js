...
test('ffmpeg-session uses formatConversionFailedMessage and Could not start FFmpeg on spawn error', () => {
    const src = read('lib/ffmpeg-session.js');
    assert.match(src, /formatConversionFailedMessage/);
    assert.match(src, /signalError\('Could not start FFmpeg'\)/);
    assert.match(src, /Could not start FFmpeg/);
    assert.doesNotMatch(src, /signalError\(err\.message\)/);
});
