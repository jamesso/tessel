...
const { STDERR_BUFFER_MAX_CHARS, appendStderrTail } = require('./stderr-tail');
...
function createFfmpegSession(deps) {
    ...
    function renameFailureMessage(tempPath) {
        const basename = tempPath.replace(/^.*[/\\]/, '');
        return `Could not save output file (left ${basename})`;
    }
    ...
    function getVideoDurationWithFFmpeg(videoPath) {
        return new Promise((resolve, reject) => {
            ...
            ffmpegProcess.stderr.on('data', (data) => {
                ...
                const parsed = matchDurationInStderr(next);
                output = appendStderrTail(output, chunk, STDERR_BUFFER_MAX_CHARS);
    ...
    function convertVideo({ vidPath1, vidPath2, vidPath3, vidPath4, vidPath5, vidPath6, vidPath7, vidPath8, vidPath9, gridType, filePath, width, height, audio, fit, padMode, durationMode, seconds }) {
        ...
        try {
            ...
            log('Valid video paths:', uniquePaths);
            ...
            const processDurations = async () => {
                ...
                try {
                    ...
                    const occupiedSlotPaths = slotPaths.filter(Boolean);
                    assertAllFiniteDurations(durations, occupiedSlotPaths);
                    videoDurations = durations;
                    const maxDuration = maxDurationFromMap(durations);
                    const encodeDuration = resolveEncodeDuration(durations, {
                        mode: durationMode,
                        seconds,
                    });
                    log('Duration analysis complete:', { maxDuration, encodeDuration, videoDurations });
                    if (myId !== currentJobId || killedByUs || probeFailed) return;
                    startConversion(encodeDuration);
                } catch (err) {
            ...
            const startConversion = (longestDuration) => {
                ...
                log('Grid configuration:', { gridType, isGrid3x3, gridSize, blockWidth, blockHeight, output, audio, fit, padMode });
                ...
                const filterComplex = buildFilterComplex(videoInfo, longestDuration, blockWidth, blockHeight, { fit, output, padMode });
                ...
                const signalError = (message, options = {}) => {
                    if (myId !== currentJobId) return;
                    if (signaled) return;
                    if (killedByUs) {
                        finishEncode();
                    ...
                    signaled = true;
                    if (activeOutputPath && !options.keepTemp) {
                ...
                ffmpegProcess.stderr.on('data', (data) => {
                    ...
                    ffmpegOutput = appendStderrTail(ffmpegOutput, output, STDERR_BUFFER_MAX_CHARS);
                ...
                ffmpegProcess.on('close', (code) => {
                    ...
                    if (code === 0) {
                        ...
                        log('=== CONVERSION END ===');
                        const renameResult = renameOutputFile(tempPath, destPath);
                        if (!renameResult.ok) {
                            signalError(renameFailureMessage(tempPath), { keepTemp: true });
                            return;
                        }
                        finishEncode();
