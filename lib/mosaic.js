...
function resolveOutput(output) {
    if (output && Number(output.width) === 1920 && Number(output.height) === 1080) {
        return { width: 1920, height: 1080, fps: 25 };
    }
    return { width: OUTPUT.width, height: OUTPUT.height, fps: OUTPUT.fps };
}
...
function buildFilterComplex(videoInfo, longestDuration, blockWidth, blockHeight, options = {}) {
    ...
    const overlayIndices = videoInfo
    ...
    let lastOutput = 'canvas';
    overlayIndices.forEach(function (index, overlayIndex) {
        const val = videoInfo[index];
        const outputName = overlayIndex === overlayIndices.length - 1
            ? 'final'
            : 'mosaic' + (overlayIndex + 1);

        complexFilter.push({
            filter: 'overlay',
            options: { x: val.coord.x, y: val.coord.y },
            inputs: [lastOutput, 'block' + index],
            outputs: outputName,
        });
