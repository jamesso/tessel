...
function buildFilterComplex(videoInfo, longestDuration, blockWidth, blockHeight, options = {}) {
    ...
    const padMode = resolvePadMode(options.padMode);
    ...
    videoInfo.forEach(function (val, index) {
        ...
        if (paddingDuration > 0.1) {
            const tpadOptions = padMode === 'freeze'
                ? `stop_mode=clone:stop_duration=${paddingDuration}`
                : `stop_mode=add:stop_duration=${paddingDuration}:color=black`;
            complexFilter.push({
                filter: 'tpad',
                options: tpadOptions,
                inputs: 'scaled' + index,
...
function buildFfmpegArgs(videoInfo, filterComplex, longestDuration, filePath, options = {}) {
    ...
    if (audio !== 'none') {
        if (audio && typeof audio === 'object') {
            const chosen = videoInfo[audio.slot];
            if (chosen && !chosen.isBlack) {
                audioSource = chosen;
            }
        }
        if (!audioSource) {
            audioSource = firstReal;
        }
    }
    const audioArgs = audioSource
        ? ['-map', `${audioSource.inputIndex}:a?`, '-af', 'asetpts=PTS-STARTPTS,apad']
        : ['-an'];
